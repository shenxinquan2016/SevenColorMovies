...
CGFloat const kTWMessageBarStyleSheetMessageBarAlpha = 0.8f;
...
CGFloat const kTWMessageBarManagerDismissAnimationDuration = 0.4f; //default is 0.25
...
@interface TWClipView : UIView

@end

@implementation TWClipView
...
- (void)title:(NSString *)title type:(TWMessageBarMessageType)type duration:(CGFloat)duration places:(NSArray *)places callBack:(void(^)())callBack {
    self.originalLocation = (NSInteger)[[places objectAtIndex:0]intValue];
    self.dispLocation = (NSInteger)[[places objectAtIndex:1]intValue];
    [self showMessageWithTitle:title description:nil type:type duration:duration callback:callBack];
}
...
#warning 可以改为clipBounds
    TWClipView *clipView = [[TWClipView alloc] initWithFrame:CGRectMake(messageView.frame.origin.x+ (kSCREEN_WIDTH-TW_WIDTH)/2, self.dispLocation-3, TW_WIDTH, TW_HEIGHT+3)];
    clipView.clipsToBounds = YES;
    clipView.backgroundColor = [UIColor clearColor];
    [clipView addSubview:messageView];
    
    CGRect mFrame = messageView.frame;
    RectSetY(mFrame, 0);
    messageView.frame = mFrame;
    [[[UIApplication sharedApplication] keyWindow] insertSubview:clipView atIndex:1];
...
//    TWMessageView *currentMessageView = nil;
    TWClipView *currentMessageView = nil;
...
//        }
        // modify by gcz 14.7.7
        if ([subview isKindOfClass:[TWClipView class]])
        {
            currentMessageView = (TWClipView *)subview;
            [currentMessageView removeFromSuperview];
        }
    }
...
- (void)showNextMessage
{
    if ([self.messageBarQueue count] > 0)
    {
        ...
        // modify by gcz 14.7.7   - self.dispLocation 原始 self.originalLocation
        // modify by gcz 14.7.8  x坐标 0 原始 ([UIScreen mainScreen].bounds.size.width-TW_WIDTH)/2
        messageView.frame = CGRectMake(0 , self.originalLocation - self.dispLocation-3, [messageView width], [messageView height]);
        //messageView.alpha = 0;
        ...
        if (messageView)
        {
            ...
            [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
                // modify by gcz 14.7.7 y坐标 0+3 原来self.dispLocation
                [messageView setFrame:CGRectMake(messageView.frame.origin.x, 0+3, [messageView width], [messageView height])]; // slide down
...
- (void)itemSelected:(id)sender
{
    ...
    if (messageView && ![messageView isHit])
    {
        ...
        TWClipView *clipView = (TWClipView *)messageView.superview;
        [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
            // modify by gcz 14.7.7 -self.originalLocation
            [messageView setFrame:CGRectMake(messageView.frame.origin.x, self.originalLocation -self.dispLocation, [messageView width], [messageView height])]; // slide back up
            // modify by lzc
            //messageView.alpha = 0;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)])
    {
        ...
        CGContextSaveGState(context);
        {
            ...
            {
                // 待开发 [[styleSheet iconImageForMessageType:self.messageType] drawInRect:CGRectMake(xOffset, yOffset, kTWMessageViewIconSize, kTWMessageViewIconSize)];
            }
        }
        CGContextRestoreGState(context);
        ...
        {
            // 动态标题高度 yOffset = ceil(rect.size.height * 0.5) - ceil(titleLabelSize.height * 0.5) - kTWMessageViewTextOffset;
        }
        [kTWMessageViewTitleColor set];
        
        [self.titleString drawInRect:CGRectMake((TW_WIDTH-titleLabelSize.width)/2, yOffset, titleLabelSize.width, titleLabelSize.height) withFont:kTWMessageViewTitleFont lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentCenter];
        
        // yOffset += titleLabelSize.height;
        // 待开发 CGSize descriptionLabelSize = [self descriptionSize];
        // 待开发 [kTWMessageViewDescriptionColor set];
        // 待开发 [self.descriptionString drawInRect:CGRectMake(xOffset, yOffset, descriptionLabelSize.width, descriptionLabelSize.height) withFont:kTWMessageViewDescriptionFont lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentLeft];
    }
}
...
//        kTWDefaultMessageBarStyleSheetErrorBackgroundColor = [UIColor colorWithRed:1.0 green:0.611 blue:0.0 alpha:kTWMessageBarStyleSheetMessageBarAlpha]; // orange
        ...
        kTWDefaultMessageBarStyleSheetErrorBackgroundColor = [UIColor colorWithRed:.0 green:0. blue:0.0 alpha:kTWMessageBarStyleSheetMessageBarAlpha]; // black
        ...
        // Colors (stroke)
        // kTWDefaultMessageBarStyleSheetErrorStrokeColor = [UIColor colorWithRed:0.949f green:0.580f blue:0.0f alpha:1.0f];
        kTWDefaultMessageBarStyleSheetErrorStrokeColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:1.0f]; // black edit by gcz 14.8.5
