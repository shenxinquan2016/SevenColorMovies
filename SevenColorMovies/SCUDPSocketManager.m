...
- (instancetype)init {
    if (self = [super init]) {
        //创建一个后台队列 等待接收数据
        dispatch_queue_t sQueue = dispatch_queue_create("My socket queue", NULL); //第一个参数是该队列的名字
        //创建一个后台队列 发送数据
        dispatch_queue_t cQueue = dispatch_queue_create("client udp socket", NULL);
        //1.实例化一个udp socket套接字对象
        // udpServerSocket需要用来接收数据
        self.udpServerSoket = [[GCDAsyncUdpSocket alloc] initWithDelegate:self delegateQueue:sQueue socketQueue:nil];
        // udpClientSocket用来发送数据
        self.udpClientSocket = [[GCDAsyncUdpSocket alloc] initWithDelegate:self delegateQueue:cQueue socketQueue:nil];
        
        //2.服务器端来监听端口9814(等待端口9814的数据)
        [self.udpServerSoket bindToPort:9814 error:nil];
        
        //3.接收一次消息(启动一个等待接收,且只接收一次)
        [self.udpServerSoket receiveOnce:nil];

        
        NSError *error = nil;
        if (![self.udpServerSoket bindToPort:PORT error:&error]) {
            NSLog(@"Error starting server (bind): %@", error);
        }
        if (![self.udpServerSoket beginReceiving:&error]) {
            [self.udpServerSoket close];
            NSLog(@"Error starting server (recv): %@", error);
        }
        // send boardcast
        if(![self.udpClientSocket enableBroadcast:YES error:&error]){
            NSLog(@"Error enableBroadcast (bind): %@", error);
        }
        NSLog(@"udp servers success starting %hu", [self.udpServerSoket localPort]);
    }
...
- (void)sendMessage:(id)message {
    ...
    [self.udpClientSocket sendData:data toHost:host port:port withTimeout:60 tag:100];
...
-(void)sendBackToHost:(NSString *)ip port:(uint16_t)port withMessage:(NSString *)s{
    ...
    [self.udpServerSoket sendData:data toHost:ip port:port withTimeout:60 tag:200];
}
