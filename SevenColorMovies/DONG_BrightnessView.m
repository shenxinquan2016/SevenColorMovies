...
@implementation DONG_BrightnessView

+ (instancetype)sharedBrightnessView {
    static DONG_BrightnessView *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[DONG_BrightnessView alloc] init];
    });
    return instance;
}
...
- (instancetype)init {
    if (self = [super init]) {
        self.frame = CGRectMake(kMainScreenWidth * 0.5, kMainScreenHeight * 0.5, 155, 155);
        ...
        self.backImage = ({
            ...
            imgView.image        = [UIImage imageNamed:@"Brightness"];
    ...
    if (self.orientationDidChange) {
        [UIView animateWithDuration:0.25 animations:^{
            if ([UIDevice currentDevice].orientation == UIDeviceOrientationPortrait
                ...
                self.center = CGPointMake(kMainScreenWidth * 0.5, kMainScreenHeight * 0.5);
            }
        } completion:^(BOOL finished) {
